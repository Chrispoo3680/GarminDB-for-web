...
class GarminConnectConfigManager(JsonConfig):
    ...
    def __init__(self, user_table):
        ...
        try:
            ...
            print(str(e))
            print(
                f"Missing or bad config: copy GarminConnectConfig.json.example from {os.path.dirname(os.path.abspath(__file__))} to {config_file}"
            )
            sys.exit(-1)
    ...
    def get_database_value(self, field, table):
        conn = sqlite3.connect(self.user_info_db_file)
        cursor = conn.cursor()
        cursor.execute("SELECT ? FROM ?", (field, table))
        result = [row[0] for row in cursor.fetchall()]
        conn.close()

        if len(result) > 1:
            raise Exception(
                f"There should only be one row in user config tables, but {len(result)} was encountered."
            )
        else:
            return result[0]
    ...
    @classmethod
    def get_user_info_db_file(cls):
        """Return the path to the user info database file."""
        return cls.get_config_dir() + os.sep + "user_info.sqlite3"
    ...
    def get_monitoring_dir(self, year, test_dir=False):
        """Return the configured directory of where the monitoring files will be stored creating it if needed."""
        return self.__create_dir_if_needed(
            self.__get_monitoring_base_dir() + os.sep + str(year)
        )

    def get_activities_dir(self, test_dir=False):
        """Return the configured directory of where the activities files will be stored."""
        return self.__create_dir_if_needed(
            self.__get_fit_files_dir(test_dir) + os.sep + "Activities"
        )
    ...
    def get_metric(self):
        ...
        return self.get_database_value("settings", "metric")
    ...
    def get_user(self):
        ...
        return self.get_database_value("credentials", "username")

    def get_password(self):
        """Return the Garmin Connect password."""
        return self.get_database_value("credentials", "password")
    ...
    def __device_garmin_dir(self):
        return self.device_mount_dir() + os.sep + "garmin"  # type: ignore
    ...
    def display_activities(self):
        ...
        activities_list = self.get_database_value("activities", "display")
        if not activities_list:
            activities_list = self.get_database_value(
                "settings", "default_display_activities"
            )
